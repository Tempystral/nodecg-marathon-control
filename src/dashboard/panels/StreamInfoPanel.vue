...
import { ActiveRunners, OBSStatus } from "@nmc/types";
...
const nextRun = computed(() =>
  allRuns.data?.find((r) => r.id === surrounding.data?.next),
);
const currentRun = computed(() =>
  allRuns.data?.find((r) => r.id === surrounding.data?.current),
);

function getActiveRunners() {
  const numRunners = currentRun.value?.teams.length;
  return activeRunners.data?.slice(0, numRunners);
}
...
function openStream(player: ActiveRunners) {
  window.open(
    `${rtmp.viewer.url}/live/key/${player.streamKey}?token=${rtmp.viewer.token}&region=${player.server}`,
    "_blank",
  );
}
...
<template>
  <Card id="runnerInfo" v-if="activeRunners.data">
    ...
    <template #content>
      <div
        class="playerDiv"
        :player="i"
        v-for="(player, i) of getActiveRunners()"
        :key="`${player}-${i}`">
        <div class="flex gap-1">
          ...
          <FloatLabel variant="on">
            ...
            <label :for="`player-${i}-server`"> View from server: </label>
          </FloatLabel>
  ...
  <Card id="runnerInfo" v-if="activeRunners.data">
    ...
    <template #content>
      <div
        v-for="team in nextRun?.teams ?? []"
        :key="team.id"
        class="flex gap-1 mt-2">
        <FloatLabel variant="on">
          <InputText
            v-model="team.players[0].social.twitch"
            disabled
        ...
        <FloatLabel variant="on">
          <Select
            v-model="team.players[0].customData.server"
            :options="servers"
            option-label="name"
            option-value="value"
            default-value="use"
          ...
          <label :for="`player-next-server`"> View from server: </label>
        </FloatLabel>
